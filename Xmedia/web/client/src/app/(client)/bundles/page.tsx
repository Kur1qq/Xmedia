                            ...
                            <div className="space-y-6 w-full">
                                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full flex flex-col lg:flex-row gap-8 lg:gap-16">
                                    ...
                                    <div className={`relative h-[250px] lg:h-[450px] max-h-[50vh] lg:w-[45%] flex-shrink-0 rounded-[24px] flex items-center justify-center ${isBooking ? 'hidden lg:block' : ''}`}>
                                        {activeBundle.image ? (
                                            <motion.img key={activeBundle.image} src={activeBundle.image} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-full max-h-full object-contain rounded-[24px] transition-all duration-500" />
                                        ) : (
                                            <div className="w-full h-full rounded-[24px] bg-zinc-800 flex items-center justify-center">
                                            ...
                                            {!isBooking ? (
                                                <motion.div key={`detail-${activeBundle.id}`} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="flex flex-col h-full py-4 pt-0">
                                                    ...
                                                    <div className="flex items-center justify-between mb-4">
                                                        <h2 className="text-2xl md:text-3xl font-bold text-white mb-0">{activeBundle.name}</h2>
