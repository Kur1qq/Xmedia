                                            ...
                                            <motion.div key={`detail-${activeService.id}`} initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -15 }} className="flex flex-col h-full py-4">
                                                ...
                                                <div className="flex flex-wrap gap-4 items-center justify-between mb-4">
                                                    <div className="flex items-center gap-3">
                                                        {activeService.mainType && <span className="flex w-fit items-center gap-2 px-2.5 py-1 bg-primary/10 border border-primary/20 rounded-full text-[10px] md:text-xs font-medium text-primary uppercase tracking-wider">{activeService.mainType.name}</span>}
                                            ...
                                            <motion.div key={`booking-${activeService.id}`} initial={{ opacity: 0, x: 15 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 15 }} className="flex flex-col h-full py-4">
                                                ...
                                                <div className="space-y-4 flex-1">
                                                    ...
                                                    <div className="pt-4 border-t border-white/10 flex items-center justify-between mt-auto">
                                                        ...
                                                        <span className="text-xl font-bold text-primary">{((Number(selectedPackages[activeService.id]?.price || 0)) * parseInt(form.duration || "0")).toLocaleString()}₮</span>
                                                    </div>
